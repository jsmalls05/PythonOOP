...
def firstPlusLength(someList):
    lastValue = someList[len(someList)-1]
    return len(someList) + lastValue
...
def lessThanSecond(someList):
    ...
    for i in range(0, len(someList), 1):
        if someList[i] < secondVal:
            newArr.append(someList[i])
    print(len(newArr))
    return newArr

print(lessThanSecond([5,2,3,2,1,4]))
